...
import com.example.mobile.adapter.AbsenceAdapter;
...
public class ManageAbsencesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Gestion du clic sur le bouton "Ajouter une absence"
        addAbsenceButton.setOnClickListener(view -> {
            Intent intent = new Intent(ManageAbsencesActivity.this, AddAbsenceActivity.class);
    ...
    private void fetchCurrentUserName() {
        ...
        if (userEmail != null) {
            ...
            db.collection("users")
                    ...
                    .whereIn("role", List.of("Agent", "Admin")) // Vérifier si le rôle est Agent ou Admin
                    .get()
                    .addOnSuccessListener(queryDocumentSnapshots -> {
                        if (!queryDocumentSnapshots.isEmpty()) {
                            for (QueryDocumentSnapshot document : queryDocumentSnapshots) {
                                String userName = document.getString("name"); // Récupérer le nom
                                userNameTextView.setText("Nom de l'agent/admin : " + userName);
                                break;
                            }
                        } else {
